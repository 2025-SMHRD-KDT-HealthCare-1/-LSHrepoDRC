...
export class ClassComponent extends Component {
  /* 
     ...
     - state, Lifecycle의 개념은 초기에 Function 컴포넌트에 존재하지 않았다
  ...
  */
  ...
  constructor(props) {
    ...
    this.handIncreament = this.handIncreament.bind(this);
    this.handDecreament = this.handDecreament.bind(this);
  }
  ...
  //   : state나 props의 값이 갱신되어, 화면이 다시 랜더링 된 직후 실행
  componentDidUpdate() {
    console.log("4. 컴포넌트가 업데이트(props나 state가 변경)");
  }

  // 5. componentWillUnmount
  //   : 컴포넌트가 제거(언마운트)되기 직전에 실행
  //   : 타이머, 이벤트 리스너, 네트워크 연결 종료 등 정리(cleanup) 할 떄 사용
  componentWillUnmount() {
    console.log("5. 컴포넌트가 언마운트(제거)");
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <button onClick={this.handIncreament}>+1</button>
        <button onClick={this.handDecreament}>-1</button>
      </div>
